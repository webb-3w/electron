...
window.setCompanyInfo = function (info, key, callback) {
  remote.dialog.setCompanyInfo(info, key, callback);
}

window.overloadURLs = function(message, p){
  var stats = fs.lstatSync(p)
  if (!stats.isDirectory()) {
    alert("资源替换路径不存在");
    return;
  }
  remote.dialog.sourceOverload(message, p);
}
